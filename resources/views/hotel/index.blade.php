...
<body class="bg-gray-50 text-gray-800 overflow-x-hidden">
  ...
  <nav class="glass-morphism fixed w-full z-50 top-0 transition-all duration-300" id="navbar">
      ...
      <div class="flex justify-between h-20 items-center">
        ...
        <div class="flex-shrink-0 flex items-center space-x-3">
          <div class="flex items-center justify-center">
            <img src="{{ asset('assets/images/logo.png') }}" alt="Logo" class="w-32 h-16 sm:w-40 sm:h-20 object-contain rounded-lg" />
          </div>
        </div>
        <!-- Desktop Menu -->
    ...
    <div id="mobile-menu" class="md:hidden hidden glass-morphism absolute top-20 left-0 right-0 w-full z-50">
      <div class="px-4 pt-2 pb-6 space-y-4 bg-white shadow-md rounded-b-xl">
        ...
        <a href="/kebijakan-privasi" class="block py-3 px-4 text-gray-700 hover:bg-yellow-50 rounded-lg transition-colors">Kebijakan dan Privasi</a>
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-4">
          ...
          <div class="flex flex-wrap gap-2 justify-center">
            <button class="filter-btn-star px-5 py-2 bg-blue-600 text-white border border-gray-200 rounded-full transition-all duration-300 font-medium text-sm active" data-filter-star="all">
              Semua Bintang
            </button>
            <button class="filter-btn-star px-5 py-2 bg-white text-gray-700 border border-gray-200 rounded-full transition-all duration-300 font-medium text-sm" data-filter-star="5-star">
              5 Bintang
            </button>
            <button class="filter-btn-star px-5 py-2 bg-white text-gray-700 border border-gray-200 rounded-full transition-all duration-300 font-medium text-sm" data-filter-star="4-star">
              4 Bintang
            </button>
            <button class="filter-btn-star px-5 py-2 bg-white text-gray-700 border border-gray-200 rounded-full transition-all duration-300 font-medium text-sm" data-filter-star="3-star">
              3 Bintang
            </button>
          </div>
          <!-- City Filter -->
          <div class="flex flex-wrap gap-2 justify-center">
            <button class="filter-btn-city px-5 py-2 bg-blue-600 text-white border border-gray-200 rounded-full transition-all duration-300 font-medium text-sm active" data-filter-city="all">
              Semua Kota
            </button>
            <button class="filter-btn-city px-5 py-2 bg-white text-gray-700 border border-gray-200 rounded-full transition-all duration-300 font-medium text-sm" data-filter-city="madinah">
              Madinah
            </button>
            <button class="filter-btn-city px-5 py-2 bg-white text-gray-700 border border-gray-200 rounded-full transition-all duration-300 font-medium text-sm" data-filter-city="makkah">
              Makkah
            </button>
          </div>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8" id="hotelList">
        @foreach ($hotels as $hotel)
          <div class="card-hover bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-lg hover:shadow-xl transform transition-all duration-500 hover:-translate-y-2 hotel-item"
               data-bintang="{{ $hotel->bintang }}"
               data-nama="{{ strtolower($hotel->nama_hotel) }}"
               data-kategori="{{ strtolower($hotel->kategori) }}">
            <!-- Image Container -->
            <div class="relative overflow-hidden rounded-xl mb-5">
              ...
              <div class="absolute top-3 right-3 bg-blue-600/80 text-white px-3 py-1 rounded-full text-xs font-semibold">
                {{ $hotel->kategori }}
              </div>
            </div>
            <!-- Content -->
  ...
  <div class="fixed bottom-6 right-6 z-50">
    <a href="https://wa.me/62818569111" target="_blank" class="w-14 h-14 bg-green-500 hover:bg-green-600 rounded-full flex items-center justify-center text-white shadow-lg hover:shadow-xl transition-all duration-300 floating-animation">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      const filterButtonsStar = document.querySelectorAll('.filter-btn-star');
      const filterButtonsCity = document.querySelectorAll('.filter-btn-city');
      ...
      menuBtn.addEventListener('click', () => {
        ...
        menuBtn.classList.toggle('text-yellow-600');
      });
      ...
      // Filter hotels based on search, star rating, and city
      function filterHotels() {
        const searchTerm = searchInput.value.toLowerCase();
        const activeFilterStar = document.querySelector('.filter-btn-star.active').dataset.filterStar;
        const activeFilterCity = document.querySelector('.filter-btn-city.active').dataset.filterCity;
        let visibleCount = 0;
        ...
        hotelItems.forEach(item => {
          ...
          const matchesStar = activeFilterStar === 'all' ||
                             (activeFilterStar === '5-star' && bintang === 5) ||
                             (activeFilterStar === '4-star' && bintang === 4) ||
                             (activeFilterStar === '3-star' && bintang === 3);
          const matchesCity = activeFilterCity === 'all' ||
                             (activeFilterCity === 'madinah' && kategori === 'madinah') ||
                             (activeFilterCity === 'makkah' && kategori === 'makkah');

          if (matchesSearch && matchesStar && matchesCity) {
      ...
      // Event listeners for star rating filter
      filterButtonsStar.forEach(button => {
        button.addEventListener('click', () => {
          filterButtonsStar.forEach(btn => btn.classList.remove('active', 'bg-blue-600', 'text-white'));
          button.classList.add('active', 'bg-blue-600', 'text-white');
          filterHotels();
        });
      });

      // Event listeners for city filter
      filterButtonsCity.forEach(button => {
        button.addEventListener('click', () => {
          filterButtonsCity.forEach(btn => btn.classList.remove('active', 'bg-blue-600', 'text-white'));
          button.classList.add('active', 'bg-blue-600', 'text-white');
          filterHotels();
        });
      });
